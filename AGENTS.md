...
2. 🚧 Windows capture backend: video path landed (`WinRecorder`: monitor → cursor-less hardware-encoded mp4, Win10 border fallback, real-screen smoke test `#[ignore]`d). Input hooks landed (`win/input.rs`: dedicated WH_MOUSE_LL/WH_KEYBOARD_LL pump thread, QPC-stamped, rebased to the first-frame epoch, mapped to surface-space and clipped to the monitor rect; key identities never read). Next: write events.jsonl beside the mp4. Note: the win-capture feature pulls in an edition-2024 dep, so building *with the feature* needs rustc ≥1.85; the ≈1.75 MSRV applies to our own code and the default feature set.
